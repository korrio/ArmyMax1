...
public class PlayerViewControllerRTMP implements IFlyMediaCallback {
	...
	public PlayerViewControllerRTMP(Activity activity, AnimationDrawable drawable,
			ImageView ivVideoLoad) {
		mainActivity = activity;
		this.ivLoad = ivVideoLoad;
	}

	public PlayerViewControllerRTMP(Activity activity, View ivVideoLoad) {
		mainActivity = activity;
		this.ivLoad = ivVideoLoad;
	}
	...
	public void startPlay(String roomTag, String roomId) {
		...
		if (roomTag.equals("0")) {
			...
			rtmpUrl = "rtmp://150.107.31.13:1935/live/" + roomId;
		} else {
			//rtmpUrl = "rtmp://videodownws.9xiu.com:1935/9xiu/" + roomId;
			rtmpUrl = "rtmp://150.107.31.13:1935/live/" + roomId;
		}
		Log.e("rtmpUrl", rtmpUrl);
		//Toast.makeText(mainActivity, rtmpUrl, Toast.LENGTH_LONG).show();
		...
		if (rtmpPlayer.start(rtmpUrl)) {
			...
			// int videoHeight=(int)
			// mainActivity.getResources().getDimension(R.dimen.video_heigth);
			// rtmpPlayer.setVideoViewSize(screenWidth,videoHeight);
		} else {
