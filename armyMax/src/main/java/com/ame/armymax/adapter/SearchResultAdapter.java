...
public class SearchResultAdapter extends ArrayAdapter<DataSearchResult>  {

	private List<DataSearchResult> planetList;
	private Context context;
	//private Filter planetFilter;
	
	
	public SearchResultAdapter(List<DataSearchResult> planetList, Context ctx) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		DataSearchResult p = planetList.get(position);
		holder.planetNameView.setText(p.getName());
		holder.distView.setText(p.getUserName());
		//holder.avatar
	...
	private class PlanetFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint == null || constraint.length() == 0) {
				...
				List<DataSearchResult> nPlanetList = new ArrayList<DataSearchResult>();
				
				for (DataSearchResult p : planetList) {
		...
		protected void publishResults(CharSequence constraint,
			...
			if (results.count == 0)
				...
				planetList = (List<DataSearchResult>) results.values;
				notifyDataSetChanged();
			}
