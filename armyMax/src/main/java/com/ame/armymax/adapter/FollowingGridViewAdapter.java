...
public final class FollowingGridViewAdapter extends BaseAdapter {
	private final Context context;
	private final List<String> urls;
	private final List<String> names;

	public FollowingGridViewAdapter(Context context) {
		this.context = context;

		// Ensure we get a different ordering of images on each run.
		urls = DataFollowing.getURLS();
		names = DataFollowing.getNAMES();
		Collections.addAll(urls);
		// Collections.addAll(urls, DataFollowing.URLS);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		String url = DataFollowing.getUrl(position);
		String name = DataFollowing.getName(position);
